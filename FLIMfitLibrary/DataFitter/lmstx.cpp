...
int lmstx(minpack_funcderstx_mn fcn, void *p, int m, int n, int s, int n_jac_group, double *x,
   ...
   *nfev = 1;
   ...
   for (;;) {
      ...
      do {
         ...
         iflag = (*fcn)(p, m, n, s, wa2, &fnorm1, wa3, 0, 0);
         ++(*nfev);
         if (iflag < 0) { goto TERMINATE; }
   ...
   double* fvec, double *fjac, int ldfjac, double *qtf, double *wa1, double *wa2, double *wa3, int n_thread)
{
   ...
   {
      for (int j = 0; j < s; j++)
         (*fcn)(p, m, n, s, x, fvec + m*j, wa3 + m*n*j, j + 2, 0);
   ...
   else
   {
      ...
      for (int i = 0; i < sm; i++)
      {
         ...
         for (int j = 0; j < j_max; j++)
            (*fcn)(p, m, n, s, x, fvec_ + m*j, wa3_ + m*n*j, i*n_jac_group + j + 2, thread);
