...
{
public:
   ExponentialPrecomputationBuffer(shared_ptr<InstrumentResponseFunction> irf, shared_ptr<DecayModel> model);
   void Compute(double rate, int irf_idx, double t0_shift, const vector<double>& channel_factors);

protected:

   void ComputeIRFFactors(double rate, int irf_idx, double t0_shift);
   void ComputeModelFactors(double rate, const vector<double>& channel_factors);
