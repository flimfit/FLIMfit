...
function h_fig = icy_im3show(h_fig, data, title)
% h_fig = icy_im3show(data)
% h_fig = icy_im3show(data, title)
% h_fig = icy_im3show(h_fig, data)
% h_fig = icy_im3show(h_fig, data, title)
%
% Display the 3D image 'data' in Icy, and return the handle of the corresponding
% figure.
%
...
if(~exist('title', 'var'))
	...
	if(isempty(title))
		counter = counter + 1;
		title   = sprintf('Image 3D %d', counter);
	end
end
...
args_in.title = title;
args_in.data  = data;
icy_command('plugins.ylemontag.matlabxserver.MatlabXServerDeamon', 'im3show', args_in);
