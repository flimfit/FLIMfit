...
const
  ...
  ITDS_DownloadFailed = 'Download failed.';

  ITDS_TitleCaption = 'Downloading additional files';
  ITDS_TitleDescription = 'Please wait while setup downloads additional files...';

  ITDS_MessageFailRetryContinue = 'Sorry, the files could not be downloaded. Check your connection and click Retry to try downloading the files again, or click Next to continue installing anyway.';
  ITDS_MessageFailRetry = 'Sorry, the files could not be downloaded. Check your connection and click Retry to try downloading the files again, or click Cancel to terminate setup';

  ITDS_Retry = 'Retry';

  {Constants for language strings of updater example}
  ITDS_Update_Caption = 'Update setup';
  ITDS_Update_Description = 'Checking for updates...';
  ITDS_Update_Checking = 'Checking for updates...';
  ITDS_Update_NewAvailable = 'There is a newer installer available. The new version is %2, the current version is %1. Would you like to download it?';
  ITDS_Update_NoNewAvailable = 'This installer is up to date.';
  ITDS_Update_WantToCheck = 'Would you like setup to check if a newer version of this program is available? (Requires an internet connection)';
  ITDS_Update_Failed = 'I was unable to check for an update, I will continue with the installation of the current version, %1.';
  ITDS_Update_WillLaunch = 'The new installer has been downloaded. It will now be launched.';
  ITDS_Update_WillLaunchWithPath = 'The new installer has been downloaded to "%1". It will now be launched.';
...
begin
  ...
  sender.caption := ITDS_TitleCaption;
  sender.description := ITDS_TitleDescription;
  ...
  case err of
    ...
    //Some unexpected error, like connection interrupted
      wizardform.backbutton.caption := ITDS_Retry;
      ...
      if itd_allowcontinue then begin //Download failed, we can retry, continue, or exit
        sender.caption := ITDS_DownloadFailed;
        sender.description := ITDS_MessageFailRetryContinue;

        MsgBox(ITDS_MessageFailRetryContinue, mbError, MB_OK)
      end else begin //Download failed, we must retry or exit setup
        sender.caption := ITDS_DownloadFailed;
        sender.description := ITDS_MessageFailRetry;

        MsgBox(ITDS_MessageFailRetry, mbError, MB_OK)
      end;
    end;
  end;
end;
...
begin
  itd_downloadpage := CreateCustomPage(afterID, ITDS_TitleCaption, ITDS_TitleDescription);
