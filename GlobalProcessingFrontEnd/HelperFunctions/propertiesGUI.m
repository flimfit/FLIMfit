function [hPropsPane,parameters] = propertiesGUI(hParent, parameters, cur_values, hid)
...
%    color, IPAddress, password, date, boolean, cell-array of strings.
%
% Inputs:
...
%
% Future development:
%    1. Enable property groups using sub-structs (params.group.field)
...
%    <a href="http://undocumentedmatlab.com/blog/jide-property-grids/">http://undocumentedmatlab.com/blog/jide-property-grids/</a>
%    <a href="http://undocumentedmatlab.com/blog/advanced-jide-property-grids/">http://undocumentedmatlab.com/blog/advanced-jide-property-grids/</a>
%
% Change log:
...
% $Revision: 1.05 $  $Date: 2012/11/07 12:28:24 $
  ...
  propsList = preparePropsList(parameters, cur_values, isEditable, hid);
  ...
  set(hPropsPane_,'tag',['hpropertiesGUI-' num2str(hid)]);
...
function parameters = demoParameters
    ...
    parameters.array_property = [11,12,13,14];
    parameters.cell_property  = {21,22,23,24};
    parameters.another_color_property = java.awt.Color.red;
end  % demoParameters

function [type,value] = getTypeValue(value)
      ...
      elseif isnumeric(value)
          if length(value)==3
              ...
              value = java.awt.Color(colorComponents{:});
          else
              value = value(1);
...
function propsList = preparePropsList(parameters, cur_values, isEditable, hid)
  propsList = java.util.ArrayList();
  
  % Prepare a dynamic list of properties, based on the struct fields5
  if isstruct(parameters) && ~isempty(parameters)
      ...
      for field_idx = 1 : length(field_names)
          ...
          [type,value] = getTypeValue(value);
          
          if isfield(cur_values,field_name)
               [new_type,new_value] = getTypeValue(cur_values.(field_name));
               if iscell(type)
                   if strcmp(new_type,'string') 
                      value = new_value;
                   end
               elseif strcmp(type,new_type)
                   value = new_value;
               end
          end          
          ...
          newProp = newProperty(parameters, field_name, field_label, isEditable, type, cval, field_description, @propUpdatedCallback, hid);
      ...
      propsList.add(newProperty(parameters, 'flag_prop_name',   'Flag value:',     isEditable, 'boolean',            'Turn this on if you want to make extra plots', @propUpdatedCallback, hid));
      propsList.add(newProperty(parameters, 'float_prop_name',  'Boolean prop',    isEditable, 'float',              'description 123...',   @propUpdatedCallback, hid));
      propsList.add(newProperty(parameters, 'string_prop_name', 'My text msg:',    isEditable, 'string',             'Yaba daba doo',        @propUpdatedCallback, hid));
      propsList.add(newProperty(parameters, 'int_prop_name',    'Now an integer',  isEditable, 'unsigned',           '123 456...',           @propUpdatedCallback, hid));
      propsList.add(newProperty(parameters, 'choice_prop_name', 'And a drop-down', isEditable, {'Yes','No','Maybe'}, 'no description here!', @propUpdatedCallback, hid));
  end
end  % preparePropsList

%% Prepare a data property
function prop = newProperty(dataStruct, propName, label, isEditable, dataType, cur_value, description, propUpdatedCallback, hid)
  ...
  if isstruct(thisProp)  %hasProperties(thisProp)
      ...
      children = toArray(preparePropsList(thisProp, cur_value, isEditable, hid));
  ...
  if iscell(dataType)
      ...
      if ~isempty(cur_value)
          idx = find(strcmp(cur_value,dataType));
          if isempty(idx)
              idx = 1;
          end
      else
      ...
      try 
          prop.setValue(dataStruct.(propName){idx}); 
      catch
          ab = 1;
      end
  else
  ...
  if prop.isEditable
      ...
      callback = @(a,b,c) propUpdatedCallback(a,b,c,hid);
      set(hprop,'PropertyChangeCallback',{callback,propName});
  else
