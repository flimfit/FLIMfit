...
template <typename T>
float quickselect(T *arr, int n, int k) 
{
...
template <typename T>
void TrimmedSum(T x[], int n, int K, T& mean, T& mean_sq)
{
   T w1, w2, OS1, OS2, wt;
...
template <typename T>
void TrimmedMean(T x[], int n, int K, T& mean, T& var)
{
   T mean_sq;
   TrimmedSum(x,n,K,mean,var);
