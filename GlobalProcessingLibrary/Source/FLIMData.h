...
template <>
double* FLIMData<double>::MemPointer<double>() { return d_data; };

template <>
uint16_t* FLIMData<uint16_t>::MemPointer<uint16_t>() { return i_data; };
...
T* FLIMData::GetDataPointer(int thread, int im)
{
   ...
   try
   {
      ...
      {
		  if(data_class == DATA_UINT16)
		      data_ptr = i_data + im * im_size;
		  else
			  data_ptr = d_data + im * im_size;
      }
   }
